...
test("CLI help interface for 'esv secret describe' Usage should be expected english", async () => {
  // Arrange
  const expected = `
  Usage: frodo esv secret describe [options] <host> [realm] [user] [password]
    `.trim();
  // Act
...
test("CLI help interface 'esv secret describe' description at line 2 should be expected english", async () => {
  // Arrange
  const expected = `
  Describe secrets.
    `.trim();
...
test("CLI help interface 'describe option -i, --secret-id <secret-id>' description should be expected english", async () => {
  // Arrange
  const expectedDescription = `
...
// frodo does not yet export and import of secrets
...
test("'-i, --secret-id <secret-id>' is a mandatory parameter", async () => {
  // Arrange
  const expected = `
  error: required option '-i, --secret-id <secret-id>' not specified
    `.trim();
  // Run command
  expect.assertions(1);
  try {
    const cmd = 'frodo esv secret describe frodo-dev';
    await exec(cmd);
  } catch (error) {
    // Act
