...
describe('frodo email template export', () => {
    ...
    test('"frodo email template export -i activation -f test.json": should export the email template with email id "activation" into file named test.json', async () => {
        const CMD = `frodo email template export -i activation -f test.json`;
    ...
    test('"frodo email template export --all": should export all email templates to a single file', async () => {
        const CMD = `frodo email template export --all`;
        const exportFile = "allEmailTemplates.template.email.json";
        await testExport(CMD, env, type, exportFile);
    });

    test('"frodo email template export -a --file test.json": should export all email templates to a single file named test.json', async () => {
        const CMD = `frodo email template export -a --file test.json`;
        const exportFile = "test.json";
        await testExport(CMD, env, type, exportFile);
    });
    ...
    test('"frodo email template export --all-separate": should export all email templates to separate files', async () => {
        const CMD = `frodo email template export --all-separate`;
        await testExport(CMD, env, type);
    });
});
