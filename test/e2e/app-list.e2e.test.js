...
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-volker-demo.forgeblocks.com/am frodo app list --long
 */
...
describe('frodo app list', () => {
    ...
    test('"frodo app list --long": should list the ids, statuses, client types, grant types, scopes, and redirect URIs of the apps', async () => {
        const CMD = `frodo app list --long`;
        const { stdout } = await exec(CMD, env);
        expect(removeAnsiEscapeCodes(stdout)).toMatchSnapshot();
    });
});
