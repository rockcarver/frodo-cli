...
- Added `--registered-client-id <client-id>` to `frodo mcp server start --oauth-resource-server`: a DCR-emulation shim (`POST /oauth2/register`, advertised via `registration_endpoint` in the served Authorization Server metadata) that hands a pre-provisioned, public (no-secret, PKCE-only) client_id back to any caller attempting Dynamic Client Registration. Neither AM-as-IdP mode's own metadata nor most external IDPs (Entra ID included) offer a `registration_endpoint` a generic MCP client can actually use — AM's own real one requires an initial access token frodo/clients don't have, and Entra ID has no RFC 7591 support at all — so without this, a DCR-attempting client has no way to obtain a client_id. Nothing is actually registered anywhere: the same operator-configured client_id is always returned, echoing back the caller's own submitted metadata. Security is unchanged from before — it still comes entirely from the real, pre-existing app registration's own redirect-URI policy on AM/the external IDP.
...
- Introduced `frodo debug journey`, an interactive, self-updating list of in-flight/recent journey executions (color-coded by status: running/finished/failed/abandoned), built from the log stream and each tree's own definition. (#688)
  ...
  - The event history also correlates `am-core` (AM's debug-level log) and `idm-access` (IDM's own audit log of every REST call) events onto the right session by transaction id -- e.g. a script node's actual exception text, or a failed `openidm` call the node made -- neither of which `am-authentication`'s audit trail ever records on its own, since AM's audit model only logs `*-COMPLETED` events, never a mid-execution failure's detail. Each event row shows a SRC column (AM/IDM) so a mixed-source failure reads at a glance.
  - Added `-i/--journey-id` and `-u/--user-id` to narrow the list to journeys whose tree name or user matches (case-insensitive substring). Every session is still tracked/cached/swept in the background regardless of whether it currently matches -- a `--user-id` filter's match target often isn't known until partway through a run, so filtering only ever narrows what's displayed, never what's ingested. `--user-id` also accepts either a username or a managed-object uuid and resolves to the other form in the background, since confirmed live which form a session actually gets recorded under is genuinely inconsistent (a plain password login's principal is the human username, `FRServiceAccountInternal`'s is a raw uuid) -- the resolved counterpart, once found, shows in the list header next to the filter.
- Added `createLogTailStream()` to `@rockcarver/frodo-lib`'s Log API and switched every log-tailing command (`frodo log tail`, every `frodo debug` topic) onto it: a stateful, deduped wrapper around `tail()` that centrally handles the audit-log tail endpoint's own documented redelivery behavior, so no command has to reimplement cookie-tracking or dedup for itself. See `@rockcarver/frodo-lib`'s own changelog for the full detail on why this exists.

### Fixed
- Fixed `frodo debug journey` sometimes showing the same node/tree-completion events twice (even a "Tree completed" line twice for one real login). Root cause: AM's tail-endpoint documentation is explicit that each poll's range "starts from the last returned log entry in the previous result (inclusive)", and confirmed live that a whole batch can be redelivered verbatim -- sometimes even within one poll's own result array -- with no event id returned on `tail()` results to tell repeats apart with. Now deduped centrally (see `createLogTailStream()` above).
...
- Fixed `frodo debug journey` never establishing an authenticated AM session when the connection profile already had cached Log API credentials (the common case after first use), silently degrading abandoned-detection to a generic fallback instead of the realm's real settings.
